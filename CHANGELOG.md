...
### Version 3.3.3  (2026.09.08)
- KOOK 语音改为强制门槛：说书人创建房间后必须先完成 KOOK 绑定（机器人 Token + 服务器），设置弹窗在绑定成功前不可关闭
- 玩家加入房间后必须先绑定自己的 KOOK 账号（用户名#识别号）才能进入；说书人尚未完成绑定时阻塞等待，可退出房间
- 创建/加入房间及链接进房不再要求先输入昵称，昵称在 KOOK 绑定后自动采用 KOOK 昵称
- 入座前校验：已绑定 KOOK 账号且处于（指定分组内的）KOOK 语音频道中才能入座，由说书人端权威执行，违规入座会被拒绝并提示
- 入座后座位昵称与头像自动采用 KOOK 信息（http 头像链接自动升级为 https）
- KOOK 语音面板标题栏跟随当前所在语音频道/所选分组显示
- KOOK 账号绑定/解绑移入 菜单 → KOOK 设置（对说书人与玩家均可用），语音面板不再内嵌绑定区
- KOOK 语音面板不再因点击外部而自动关闭，仅手动关闭或按 Esc 关闭；频道列表使用上下滚动条
- 新增主频道：说书人设置分组后可指定一个语音子频道为主频道，玩家绑定 KOOK 后第一次进入该分组的语音频道会被自动拉入主频道
- 新增频道邀请：语音面板频道行新增邀请按钮，可多选主频道内的玩家发送邀请（邀请信息含邀请人、目标频道与全部被邀请人），接受后自动移动到该频道
